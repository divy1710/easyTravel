    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 py-8 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-xl px-4 py-2 rounded-full mb-4">
              <Sparkles className="w-5 h-5 text-yellow-400" />
              <span className="text-white/80 text-sm font-medium">AI-Powered Planning</span>
            </div>
          ...
          <motion.div
            ...
            className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8 relative overflow-hidden"
          >
            {/* Loading Overlay */}
            {mutation.isPending && (
              <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center z-10 rounded-3xl">
                <div className="relative mb-6">
                  ...
                  <Plane className="absolute inset-0 m-auto w-8 h-8 text-indigo-400" />
                </div>
                <p className="text-xl font-semibold text-white mb-2">Creating your perfect itinerary…</p>
                <p className="text-white/60">Our AI is planning your adventure</p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    />
                    <div className="w-16 text-center bg-white/10 rounded-xl px-3 py-2 text-white font-semibold">
                ...
                <div>
                  <label className="flex items-center gap-2 text-white font-medium mb-3">
                    <SeasonIcon className="w-5 h-5 text-indigo-400" />
                    Travel Month
                  </label>
                  <select
                    value={month}
                    onChange={(e) => setMonth(e.target.value)}
                    className="w-full bg-white/10 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                  >
                    {MONTHS.map((m) => (
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white/10 border border-white/10 rounded-xl p-4 text-white placeholder-white/40 focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all pr-10"
                  />
                  {isSearching && (
                ...
                <AnimatePresence>
                  {showSuggestions && suggestions.length > 0 && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute z-50 w-full mt-2 bg-slate-800/95 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-2xl"
                    >
                      {suggestions.map((suggestion) => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-white/50 text-sm truncate">{suggestion.fullName}</p>
                          </div>
                        </button>
                      ))}
                    </motion.div>
              ...
              <motion.button
                ...
                className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-4 rounded-2xl font-semibold text-lg hover:shadow-lg hover:shadow-purple-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
              >
